...
void *run(void *args){
    ...
    jshortArray diffdata = env->NewShortArray(DIFF_LENGTH+1);
        ...
        for(int i=0;i<nfds;++i)
        {
            if(events[i].events & EPOLLIN)//
            {
                ...
                if(sockfd == fd)  //diffData
                {
                    ...
                    //new add
                    int16_t rawdata_after[32 * 18+1];
                    for (int i = 0; i < 32 * 18; i++)
                    {
                        rawdata_after[i] = (jshort)(frameData.rawData[i]);
//                        rawdata_after[i] = (jshort)(frameData.diffData[i]);
                    }
